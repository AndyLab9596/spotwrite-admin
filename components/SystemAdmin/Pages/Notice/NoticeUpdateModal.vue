            ...
            <validation-observer v-else v-slot="{ errors, invalid, passes }">
              <div class="radio-group">
                ...
                <label
                  for="type-notice"
                  :class="{
                    checked: updateNotice.type == 'notice',
                    disabled: !updateNotice.is_draft,
                  }"
                >
                  <input
                    id="type-notice"
                    v-model="updateNotice.type"
                    :disabled="!updateNotice.is_draft"
                    type="radio"
                    name="type"
                    value="notice"
                ...
                <label
                  for="type-update"
                  :class="{
                    checked: updateNotice.type == 'update',
                    disabled: !updateNotice.is_draft,
                  }"
                >
                  <input
                ...
                <button v-if="updateNotice.is_draft" @click="saveDraft()">
                  下書き保存する
                </button>
              </div>
...
import { fetchNotice } from '@/libs/api';

type UpdateNotice = ApiRequest.Notice;
...
export default Vue.extend({
  ...
  methods: {
    ...
    onSubmit() {
      this.isLoading = true;
      this.updateNotice.is_draft = 0;
      this.$emit('submit', this.uuid, { ...this.updateNotice });
    },
    saveDraft() {
      this.isLoading = true;
      this.updateNotice.is_draft = 1;
      this.$emit('submit', this.uuid, { ...this.updateNotice });
    },
    async fetchNotice() {
      try {
        const { data } = await fetchNotice(this.uuid);
        this.updateNotice = {
          type: data.type,
          is_draft: data.is_draft,
        };
        if (data.title.length) {
          this.updateNotice.title = data.title;
        }
        if (data.message.length) {
          this.updateNotice.message = data.message;
        }
      } catch (e) {
        throw new Error(e);
      } finally {
