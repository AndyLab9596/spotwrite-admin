<template>
  <div class="store-profile-container">
    <ValidationObserver v-slot="{ handleSubmit, invalid }">
      ...
      >
        <validation-provider v-slot="name" rules="required" name="支店・店舗名">
          <div class="input-box">
            <label class="label">支店・店舗名</label>
            <input
              id="name"
              v-model="input.name"
              type="text"
        ...
        <validation-provider v-slot="description" name="店舗概要">
          <div class="textarea-box form-item">
            <label class="label">支店・店舗概要</label>
            <base-textarea
              v-model="input.description"
              class="textarea"
              :class="{ 'input-error': descriptionExceeded }"
              placeholder="支店・店舗についてまとめた文章を入力してください。"
            ></base-textarea>
          </div>
        ...
        <div v-if="input.leader_uuid" class="textarea-box form-item">
          <label class="label">店長メッセージ</label>
          <base-textarea
            v-model="input.message"
            class="textarea"
            placeholder="店長からのメッセージを入力してください。"
          ></base-textarea>
        </div>

        <validation-provider v-slot="address" name="所在地">
          <div class="input-box form-item">
            <label class="label">所在地</label>
            <base-input-text
              v-model="input.address"
              class="input"
              placeholder="例）東京都中央区晴海3-13-1 DEUX TOURS EAST TOWER 4620"
            ></base-input-text>
          </div>
          <span v-if="address.errors[0]" class="error"
        ...
        <validation-provider v-slot="homepage" name="ホームページ">
          <div class="input-box form-item">
            ...
            <base-input-text
              ...
              class="input"
              placeholder="例）http://worksmedia.jp"
            ></base-input-text>
          </div>
...
export default Vue.extend({
  ...
  mounted() {
    ...
    this.input = {
      ...
      address: this.$props.organization.detail.address,
