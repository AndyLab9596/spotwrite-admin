...
export default Vue.extend({
  ...
  props: {
    ...
    propsGroup: {
      type: Object as PropType<Types.OrganizationGroup>,
      required: true,
    },
  },
  data() {
  ...
  created() {
    // 既にチェックされているグループのuuidを保持する
    this.oldChecked = this.propsGroup.organizations.map(
      (o: Types.Organization) => o.uuid
    );
    // 表示させるためのグループのuuid
    this.checked = this.propsGroup.organizations.map(
      (o: Types.Organization) => o.uuid
    );
  },
  methods: {
    ...
    add() {
      const organizationUuids = this.checked.filter((c: string) => {
        return !this.oldChecked.some((o: string) => o === c);
      });
      const data: {
        ...
        group: this.propsGroup,
        checkedOrganizationUuids: this.checked,
        organizationUuids,
      };
      this.$emit('add', data);
      this.checked = [];
