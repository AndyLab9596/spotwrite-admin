    ...
    <div class="modal modal-overlay" @click.self="onClose">
      <div class="modal-window">
        <div class="modal-header">
          <h3 class="modal-header-title">サブ組織を設定</h3>
        ...
        <div class="modal-content">
          ...
          <div class="choices choices__radiobox">
            <p
              v-for="(organization, index) in selectedOrganizations"
              :key="`filter${index}`"
            >
              <template v-if="childOrganizations(organization.uuid)">
                <label class="organization-name">
                  <base-checkbox
                    ...
                    class="checkbox"
                    :disabled="
                      organization.uuid === selectedMember.organization
                    "
              ...
              <base-checkbox
                ...
                :disabled="organization.uuid === selectedMember.organization"
                @change="changeSelectedValues"
              />
            </p>
          </div>
          <h3 class="select-items-title">選択したサブ組織（BOX）</h3>
          ...
          <div class="button-area">
            <button class="button" @click="add">保存する</button>
          </div>
        </div>
      </div>
    </div>
...
export default Vue.extend({
  ...
  data() {
    return {
      checkedAllOrganization: false as boolean,
  ...
  computed: {
    selectedOrganizations(): Types.GroupOrganization[] {
  ...
  created() {
    this.companyOfMyOrganizations = this.organizations
      ...
      .map((item: Types.GroupOrganization) => ({
        uuid: item.uuid,
        name: item.name,
      }));
    ...
    this.selectedValues = this.selectedMember.sub_organizations.map(
  ...
  methods: {
    ...
    toggleAllCheck(arr: string[]) {
      this.selectedValues = arr.length
    ...
    organizationName(uuid: string): string {
      const element = this.organizations.find((o: Types.GroupOrganization) => {
        return o.uuid === uuid;
      });
    ...
    companyName(uuid: string): string {
      ...
      const element = this.organizations.find(
        (o: Types.GroupOrganization) => o.uuid === uuid
      );

      if (element != null && element.relations.company) {
        companyUuid = element.relations.company;
      } else {
        companyUuid = '';
      }

      const companyElement = this.companyOfMyOrganizations.find(
        (c: TabMenu) => c.uuid === companyUuid
      );
      if (companyElement != null) return companyElement.name;
      return '';
    },
    ...
    add() {
      const data: ApiResponse.User = {
        ...this.selectedMember,
        sub_organizations: this.selectedValues,
      };
      this.$emit('add', data);
      this.selectedValues = [];
      this.$emit('close');
