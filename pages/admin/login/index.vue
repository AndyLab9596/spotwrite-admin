  ...
  <div class="page-container">
    <ValidationObserver v-slot="{ handleSubmit }">
      <form class="form" @submit.prevent="handleSubmit(onSubmit)">
        ...
        <nuxt-link
          to="/admin/reset-password/send-email"
    ...
    <div class="register-box">
      <nuxt-link to="/admin/register" class="register-link">
        <span class="register-text">新規登録はこちら</span>
        <img class="arrow-right" src="/icon/icon_arrow_right_white@2x.png" />
      </nuxt-link>
    </div>
  </div>
</template>
...
export default Vue.extend({
  ...
  methods: {
    async onSubmit() {
      this.isLoading = true;
      try {
        const { data } = await loginEmployee(this.input);
    ...
    async fetchUserProfile(): Promise<void> {
      ...
      if (data.has_same_account_holder_email) {
